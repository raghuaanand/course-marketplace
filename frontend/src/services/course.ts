import { Course, CourseStatus, Category, Module, Lesson, Review } from '@course-marketplace/shared';
...
class CourseService {
  ...
  async getCourseBySlug(slug: string): Promise<Course> {
    return apiService.get<Course>(`/courses/slug/${slug}`);
  }
  ...
  async getInstructorCourses(filters?: Partial<CourseFilters>): Promise<CoursesResponse> {
    ...
    return apiService.get<CoursesResponse>(`/courses/instructor/my?${params.toString()}`);
  }
  ...
  // Reviews
  async getCourseReviews(courseId: string, page = 1, limit = 10): Promise<{ reviews: Review[]; total: number }> {
    return apiService.get<{ reviews: Review[]; total: number }>(`/courses/${courseId}/reviews?page=${page}&limit=${limit}`);
  }
